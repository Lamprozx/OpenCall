...
func TestRegisterLoopFlag(t *testing.T) {
	...
	p := &playOptions{}
	p.register(fs)
	if err := fs.Parse(reorderArgs([]string{"6281", "--loop", "3"})); err != nil {
	...
	p2 := &playOptions{}
	p2.register(fs2)
	if err := fs2.Parse(reorderArgs([]string{"6281", "--loop"})); err == nil {
	...
	p3 := &playOptions{}
	p3.register(fs3)
	if err := fs3.Parse(reorderArgs([]string{"6281", "--loop", "abc"})); err == nil {
...
func TestOpenClientStore(t *testing.T) {
	...
	if len(r.Sessions) > 0 {
		...
		dbPath = storePath(best.ID)
	} else if _, err := os.Stat(legacyDB); err == nil {
		dbPath = legacyDB
	}
	if dbPath == "" {
		skipFixture(t, "session store (run `auth` first)")
	}
	logger := zerolog.Nop()
