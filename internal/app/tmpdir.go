...
// TempDir resolves a writable directory for converted media, falling back to a
// local .tmp dir that is auto-removed when the call ends.
...
// CleanupTempDir removes the fallback temp dir if one is active.
func CleanupTempDir() {
	tempState.mu.Lock()
	defer tempState.mu.Unlock()
...
// ResetTempDirState clears the cached temp dir resolution (used by tests).
