...
// Picker is a raw-mode interactive list/confirm/line-editor for session flows.
type Picker struct {
	in  *bufio.Reader
	out io.Writer
...
func (pt *Picker) hideCursor() { fmt.Fprint(pt.out, "\033[?25l") }
func (pt *Picker) showCursor() { fmt.Fprint(pt.out, "\033[?25h") }
...
func (pt *Picker) ensureRoom(needed int) {
...
func (pt *Picker) drawBlock(lines []string) {
	for i, l := range lines {
		if i > 0 {
			fmt.Fprint(pt.out, "\n")
		}
...
func (pt *Picker) redrawBlock(lines []string) {
	old := pt.block
	if old == 0 {
		pt.drawBlock(lines)
		return
...
func (pt *Picker) arrowKey() (dir byte, ok bool) {
...
func (pt *Picker) PickList(title, header string, items []string, footer string) (idx int, key byte, ok bool) {
	if len(items) == 0 {
		return 0, 'q', false
	}
	idx = 0
...
func (pt *Picker) drawList(title, header string, items []string, footer string, idx int) {
	vis := len(items)
...
func (pt *Picker) Confirm(question, yesLabel, noLabel string) (yes, ok bool) {
...
func (pt *Picker) EditLine(prompt, initial, notice string) (string, bool) {
	buf := []rune(initial)
	cursor := len(buf)
	pt.showCursor()
	pt.ensureRoom(3)
