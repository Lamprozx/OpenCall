...
func consoleLoop(ctx context.Context, cancel context.CancelFunc, state *callState) {
	log := zerolog.Ctx(ctx)
	ui := termUI
	if ui != nil {
	...
	fmt.Fprintln(termOut, "console ready — type 'help' for commands")
	if ui == nil {
...
func consoleExec(ctx context.Context, cancel context.CancelFunc, log *zerolog.Logger, state *callState, input string) bool {
	...
	case "loglevel":
		...
		consoleMinLevel.Store(int32(lvl))
