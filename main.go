...
func main() {
	...
	logger := newLogger(termOut, resolveLogLevel(levelArg, quiet), !showNoise, rec)
	installLibsignalLogger(logger)
	...
	if rec != nil {
		...
		ctx = context.WithValue(ctx, diagCtxKey{}, rec)
	}
...
func newLogger(out io.Writer, level zerolog.Level, filterNoise bool, rec *diag.Recorder) zerolog.Logger {
	var w io.Writer = out
	if filterNoise {
		w = &noiseFilter{out: out}
	}
	console := zerolog.ConsoleWriter{Out: w, TimeFormat: "15:04:05.000", NoColor: termUI != nil}
